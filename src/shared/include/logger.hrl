...
-define(ERROR(Msg, Args), rebar_log:log(error, lists:concat(["~s ~s~w " , Msg, "~n"]),[pid_to_list(self()), lists:concat([?MODULE, ".erl:"]), ?LINE | Args])).
-define(INFO(Msg, Args), rebar_log:log(info, lists:concat(["~s ~s~w " , Msg, "~n"]),[pid_to_list(self()), lists:concat([?MODULE, ".erl:"]), ?LINE | Args])).
-define(WARN(Msg, Args), rebar_log:log(warn, lists:concat(["~s ~s~w " , Msg, "~n"]),[pid_to_list(self()), lists:concat([?MODULE, ".erl:"]), ?LINE | Args])).
-define(DEBUG(Msg, Args), rebar_log:log(debug, lists:concat(["~s ~s~w " , Msg, "~n"]),[pid_to_list(self()), lists:concat([?MODULE, ".erl:"]), ?LINE | Args])).
%%     
